...
public class RenameKodiEpisodesTask extends SwingWorker<Void, String> {
	...
	private final Map<String, String> fEpisodeMap;
	...
	private File rename(String episodeKey, File file) {
		...
		return isOk ? newFile : file;

	}

	private Map<String, File> buildFileMap() {
		Map<String, File> fileMap = new HashMap<>();
		for (File file : Objects.requireNonNull(fDirectory.listFiles())) {
			if (!file.isFile()) {
				continue;
			}
